...
// // ➕ Add Teacher (multi-insert support)
// app.post('/add-teacher', async (req, res) => {
//   const { teacherName, className, subject, periods } = req.body;

//   if (!teacherName || !className || !subject || !periods) {
...
//       const per = parseInt(periodsArray[i]);

//       if (!cls || !sub || isNaN(per) || per <= 0) continue;

//       await dbRun(
//         'INSERT INTO teachers (teacherName, className, subject, periods) VALUES (?, ?, ?, ?)',
//         [teacherName.trim(), cls, sub, per]
//       );
//     }
...
//   try {
//     const rows = await dbAll('SELECT * FROM teachers');
//     res.render('teachers', { teachers: rows });
//   } catch (err) {
//     res.send('Error fetching teachers.');
//   }
// });
...
//   } catch (err) {
//     res.send('Error loading teacher.');
//   }
// });
...
//   if (!teacherName || !className || !subject || isNaN(periods) || periods <= 0) {
//     return res.status(400).send("Invalid input.");
//   }
...
//       [teacherName, className.trim(), subject.trim(), parseInt(periods), id]
//     );
//     res.redirect('/teachers');
//   } catch (err) {
...
// // 📅 Class Timetable Generation (with DB save)
// app.get('/generate', (req, res) => {
//   db.all('SELECT * FROM teachers', [], async (err, teachers) => {
//     if (err) return res.send('Error retrieving teacher data.');
...
//     const teacherSlots = teachers.map(t => ({
//       ...t,
//       remaining: t.periods
//     }));
...
//     // Clear previous entries from timetable_entries
//     db.run('DELETE FROM timetable_entries', [], (err) => {
//       if (err) console.error('Failed to clear old timetable_entries:', err.message);
//     });
...
//           const teacherAvailable = teacherTimetables[selected.teacherName][d][p] === null;
//           const classSlotEmpty = classTimetables[cls][d][p] === null;

//           if (teacherAvailable && classSlotEmpty) {
//             const slot = {
//               className: cls,
...
//             // ✅ Save this slot to timetable_entries
//             db.run(`
//               INSERT INTO timetable_entries (className, teacherName, subject, day, period)
//               VALUES (?, ?, ?, ?, ?)
//             `, [slot.className, slot.teacherName, slot.subject, slot.day, slot.period], (err) => {
//               if (err) console.error('Failed to save timetable entry:', err.message);
//             });
//           }
//         }
//       }
...
//           if (!teacherTimetables[selected.teacherName][d][p] && !classTimetables[cls][d][p]) {
...
//   } catch (err) {
//     res.send('Error generating teacher timetables.');
//   }
// });

// // 📥 PDF Download (Teacher + Class)
...
//       'Content-Disposition': 'attachment; filename="teacher-timetables.pdf"',
//     });
...
//     res.set({
//       'Content-Type': 'application/pdf',
//       'Content-Disposition': 'attachment; filename="class-timetables.pdf"',
//     });
